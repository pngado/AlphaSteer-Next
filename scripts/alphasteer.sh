...
for file in $TRAIN_VAL_DIR/*.json; do
    ...
    echo "Extracting embeddings for $filename"
    
    python src/extract_embeddings.py --model_name $MODEL_NAME \
                                    --input_file $TRAIN_VAL_DIR/$file \
...
for file in $GENERATE_CONFIG_DIR/*.yaml; do
    ...
    echo "Generating response for $filename"
    python src/generate_response.py --config_path $GENERATE_CONFIG_DIR/$file
done