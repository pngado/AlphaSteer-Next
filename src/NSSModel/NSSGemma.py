...
class NSSGemma2DecoderLayer(Gemma2DecoderLayer):
...
class NSSGemma2Model(Gemma2Model):
    def __init__(self, config: Gemma2Config):
        super().__init__(config)
        # Replace the layers with our custom NSS layers, keeping everything else unchanged
        self.layers = nn.ModuleList(
            [NSSGemma2DecoderLayer(
                config=config, 
                layer_idx=layer_idx, 
            )
...
class NSSGemma2ForCausalLM(Gemma2ForCausalLM):
    def __init__(self, config: Gemma2Config):
        super().__init__(config)
        self.model = NSSGemma2Model(config=config)
