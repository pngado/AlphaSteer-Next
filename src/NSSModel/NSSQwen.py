...
class NSSQwen2DecoderLayer(Qwen2DecoderLayer):
...
class NSSQwen2Model(Qwen2Model):
    def __init__(self, config: Qwen2Config):
        super().__init__(config)
        self.layers = nn.ModuleList(
            [NSSQwen2DecoderLayer(
                config=config, 
                layer_idx=layer_idx, 
            )
...
class NSSQwen2ForCausalLM(Qwen2ForCausalLM):
    def __init__(self, config: Qwen2Config):
        super().__init__(config)
        self.model = NSSQwen2Model(config=config)
